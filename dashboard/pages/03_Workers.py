...
for worker, df in worker_groups:
    ...
    avg_hold = (df["closed_at"] - df["opened_at"]).dt.total_seconds().mean() / 3600 if not df.empty else 0
    rows.append(
        {
            "worker": worker,
            ...
            "symbol": df["symbol"].mode().iloc[0] if not df["symbol"].empty else "",
        }
    )
...
def _worker_form(worker_key: str) -> Dict:
    ...
    max_pos = st.number_input("Max position", value=float(worker_conf.max_position), min_value=0.0, step=0.1, key=f"maxpos_{worker_key}")
    risk = st.number_input("Risk per trade", value=float(worker_conf.risk_per_trade), min_value=0.0, max_value=0.2, step=0.005, key=f"risk_{worker_key}")
    ...
    for key, val in worker_conf.parameters.items():
        params[key] = st.text_input(f"Param {key}", value=str(val), key=f"param_{worker_key}_{key}")
