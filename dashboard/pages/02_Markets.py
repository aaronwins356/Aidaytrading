"""Market structure and trade execution page."""
...
def _imbalance(orderbook: pd.DataFrame) -> float:
    if orderbook.empty:
        return 0.0
    return float((orderbook["bid"].sum() - orderbook["ask"].sum()) / (orderbook["bid"].sum() + orderbook["ask"].sum()))
...
start, end = filters.get("date_range", (pd.Timestamp.utcnow().date() - pd.Timedelta(days=7), pd.Timestamp.utcnow().date()))
...
chart_trades = trades[trades["symbol"] == symbol] if not trades.empty else pd.DataFrame()
...
if "ATR" in overlay_toggle and not market_df.empty:
    ...
    chart.add_trace(go.Scatter(x=market_df["ts"], y=market_df["close"] + atr * atr_mult, name="ATR Upper", line=dict(color="#22d3ee", dash="dot")))
    chart.add_trace(go.Scatter(x=market_df["ts"], y=market_df["close"] - atr * atr_mult, name="ATR Lower", line=dict(color="#22d3ee", dash="dot")))
...
if "Bollinger" in overlay_toggle and not market_df.empty:
    ...
    chart.add_trace(go.Scatter(x=market_df["ts"], y=mid + bb_dev * std, name="BB Upper", line=dict(color="#f97316", dash="dot")))
    chart.add_trace(go.Scatter(x=market_df["ts"], y=mid - bb_dev * std, name="BB Lower", line=dict(color="#f97316", dash="dot")))
...
with col2:
    ...
    gauge = go.Figure(go.Indicator(mode="gauge+number", value=imb * 100, gauge={"axis": {"range": [-100, 100]}, "bar": {"color": "#7C3AED"}}, title={"text": "Bid-Ask %"}))
...
if not chart_trades.empty:
    ...
    st.dataframe(chart_trades[["trade_id", "opened_at", "side", "qty", "entry", "exit", "pnl", "worker"]].sort_values("opened_at", ascending=False))
    st.download_button("Export trades CSV", data=chart_trades.to_csv(index=False), mime="text/csv", file_name=f"{symbol}_trades.csv")
